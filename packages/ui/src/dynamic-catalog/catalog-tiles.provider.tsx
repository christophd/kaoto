...
import { camelComponentToTile, camelEntityToTile, camelProcessorToTile, citrusComponentToTile, kameletToTile } from '../camel-utils';
...
export const CatalogTilesProvider: FunctionComponent<PropsWithChildren> = (props) => {
  ...
  const fetchTiles = useCallback(async () => {
    const [componentsCatalog, patternsCatalog, entitiesCatalog, kameletsCatalog, testActions, testContainers, testEndpoints] = await Promise.all([
    ...
    Object.values(testContainers ?? {}).forEach((action) => {
      combinedTiles.push(citrusComponentToTile(action));
    });
    Object.values(testEndpoints ?? {}).forEach((action) => {
      combinedTiles.push(citrusComponentToTile(action));
    });
