...
export class CitrusTestSchemaService {
  ...
  static getTestActionDefinition(actionName: string): ICitrusComponentDefinition | undefined {
    const actionDef: ICitrusComponentDefinition | undefined = CamelCatalogService.getComponent(CatalogKind.TestAction, actionName) ??
  ...
  private static resolveTestActionGroup(group: string, actionDef: ICitrusComponentDefinition): ICitrusComponentDefinition {
    let groupDef: ICitrusComponentDefinition | undefined = CamelCatalogService.getComponent(CatalogKind.TestAction, group);
    ...
    if (actionDef.propertiesSchema) {
        const schemaProperties = actionDef.propertiesSchema?.properties || {} as Record<string, KaotoSchemaDefinition["schema"]>;
        for (let propertiesKey in groupDef?.propertiesSchema?.properties) {
