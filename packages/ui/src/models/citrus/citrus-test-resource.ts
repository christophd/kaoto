...
import { AddStepMode, BaseVisualCamelEntityConstructor, IVisualizationNodeData } from '../visualization/base-visual-entity';
...
export class CitrusTestResource implements CamelResource {
  ...
  static readonly PARAMETERS_ORDER = ['name', 'author', 'status', 'description', 'endpoints', 'variables', 'actions', 'finally'];
  ...
  getVariables(): string[] {
    ...
    test.actions.forEach((action) => {
      if (action.createVariables !== undefined) {
