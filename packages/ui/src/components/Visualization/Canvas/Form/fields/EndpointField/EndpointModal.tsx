...
export type EndpointModalProps = {
  ...
  onConfirm: (type: string, model: any) => void;
  onCancel: () => void;
};
...
export const EndpointModal: FunctionComponent<EndpointModalProps> = ({
  ...
  return (
    <Modal
      ...
      data-testid="EndpointModal"
      onClose={onCancel}
      ouiaId="EndpointModal"
    >
      <ModalHeader
        title={`${mode} endpoint`}
        ...
        <ModelContextProvider model={endpointModel} onPropertyChange={onEndpointModelChange}>
          <SchemaProvider schema={endpointsSchema}>
            <CanvasFormTabsContext.Provider value={formTabsValue}>
              <EndpointFieldInner propName={ROOT_PATH}
                                  model={endpointModel}
      ...
      <ModalFooter>
        <Button key="confirm" variant="primary" isDisabled={endpointModel === undefined} onClick={handleConfirm} data-testid="endpoint-modal-confirm-btn">
          {mode}
        </Button>
