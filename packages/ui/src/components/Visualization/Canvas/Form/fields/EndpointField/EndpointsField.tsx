...
export const EndpointsField: FunctionComponent<FieldProps> = ({
  propName,
  required,
}) => {
  const { value = [], onChange } = useFieldValue<Record<string, any>[]>(propName);
  ...
  const [items, setItems] = useState<{ name: string; type: string }[]>(endpointsHandler.getDefinedEndpointsNameAndType(value));
  const renameEndpointModalContext = useContext(ActionConfirmationModalContext);

  const handleCreateOrEdit = useCallback(async (type: string, model: any) => {
    ...
    [endpointsHandler, editModel, operation, renameEndpointModalContext],
  );
  ...
  return (
    <>
      ...
      >
        ...
        <Button icon={<PlusCircleIcon />} key="create" variant="link" onClick={handleAdd} data-testid="create-new-endpoint-btn">
          Add
        </Button>
      </FieldWrapper>
